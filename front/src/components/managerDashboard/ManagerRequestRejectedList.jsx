...
const ManagerRequestRejectedList = () => {
    const [request, setRequestByManageRejected] = useState([]);
    ...
    const getRequestByManageRejected = async () => {
        const response = await axios.get("http://localhost:5000/requestByManageRejected");
        setRequestByManageRejected(response.data);
    };

   
    useEffect(() => {
        const fetchData = async () => {
          const res = await axios.get(`http://localhost:5000/requestByManageRejected/?q=${query}`);
          setRequestByManageRejected(res.data);
        };
